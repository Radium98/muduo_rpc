...
void  RpcProvider::NotifyService(google::protobuf::Service *service)
{
    ...
    // std::cout << "service name is: " << service_name << std::endl;
    ...
    for(int i = 0; i < methodCnt; i ++ )
    {
        ...
        // std::cout << "method name is: " << method_name << std::endl;
...
void RpcProvider::Run()
{
    ...
    for(auto &sp : m_serviceMap)
    {
        ...
        {
            //  service_name/method_name            /UserServiceRpc/Login  存储当前此Rpc服务节点主机的ip与port
...
/*
在框架内部，RpcProvider和RpcConsumer协商好彼此之间通信用的protobuf数据类型
service_name method_name args    定义proto的message类型，进行数据头的序列化和反序列化
head_size(4 byte) + head_str + args_str
*/
                            ...
                            muduo::Timestamp)
{
    ...
    //  打印调试信息
    std::cout << "==========================================" << std::endl;
    std::cout << "header_size is: " << header_size << std::endl;
    std::cout << "rpc_header_str is: " << rpc_header_str << std::endl;
    std::cout << "service_name is: " << service_name << std::endl;
    std::cout << "method_name is: " << method_name << std::endl;
    std::cout << "args_str is: " << args_str << std::endl;
    std::cout << "==========================================" << std::endl;
...
//  Closure回调操作，用于序列化rpc的响应与网络发送
void  RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr& conn, google::protobuf::Message* response)
{
    std::string response_str;
    if(response -> SerializeToString(&response_str)) // response进行序列化
    {
        //  序列化成功后，将rpc执行的结果发送给rpc调用方
    ...
    conn -> shutdown(); //  模仿http的短链接服务，由rpcprovider主动断开连接
}